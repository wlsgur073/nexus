...
- 유틸: `getSolutionBySlug()`, `getSolutionsByCategory()`, `getCategoryById()`, `DynamicIcon`, `getConnectionsByNode()`, `getActiveConnections()`
- 허브 연결: `HubConnection` 타입, `hubConnections` 배열 (Hub 대시보드 노드 그래프용)
...
- 3개 컴포넌트: PlatformShell, Header, ThemeToggle (Sidebar, MobileSidebar, Breadcrumbs 삭제됨 — Hub-Centric Layout 전환)
